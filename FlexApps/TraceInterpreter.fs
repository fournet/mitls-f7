...
no of repeated web traces = 350 
no of all web traces = 445 
no of client_cert web traces = 114 
no of client_nocert web traces = 51 
no of server web traces = 280 
*)
let cfuns = ref [];
...
cfuns := (121,tr121)::!cfuns;


let tr122 st chain salg skey = 
  let log = empty_bytes in
  let fch = {FlexConstants.nullFClientHello with ciphersuites = Some([TLS_DHE_RSA_WITH_AES_128_CBC_SHA]) } in
  let st,nsc,fch   = FlexClientHello.send(st,fch) in
  let log = log @| fch.payload in
...
cfuns := (122,tr122)::!cfuns;


let tr123 st chain salg skey = 
  let log = empty_bytes in
  let fch = {FlexConstants.nullFClientHello with ciphersuites = Some([TLS_DHE_RSA_WITH_AES_128_CBC_SHA]) } in
  let st,nsc,fch   = FlexClientHello.send(st,fch) in
  let log = log @| fch.payload in
...
cfuns := (127,tr127)::!cfuns;


let tr128 st chain salg skey = 
  let log = empty_bytes in
  let fch = {FlexConstants.nullFClientHello with ciphersuites = Some([TLS_RSA_WITH_AES_128_CBC_SHA]) } in
  let st,nsc,fch   = FlexClientHello.send(st,fch) in
  let log = log @| fch.payload in
...
cfuns := (128,tr128)::!cfuns;


let tr129 st chain salg skey = 
  let log = empty_bytes in
  let fch = {FlexConstants.nullFClientHello with ciphersuites = Some([TLS_RSA_WITH_AES_128_CBC_SHA]) } in
  let st,nsc,fch   = FlexClientHello.send(st,fch) in
  let log = log @| fch.payload in
...
cfuns := (160,tr160)::!cfuns;


let tr161 st chain salg skey = 
  let log = empty_bytes in
  let fch = {FlexConstants.nullFClientHello with ciphersuites = Some([TLS_RSA_WITH_AES_128_CBC_SHA]) } in
  let st,nsc,fch   = FlexClientHello.send(st,fch) in
  let log = log @| fch.payload in
...
cfuns := (161,tr161)::!cfuns;


let tr162 st chain salg skey = 
  let log = empty_bytes in
  let fch = {FlexConstants.nullFClientHello with ciphersuites = Some([TLS_RSA_WITH_AES_128_CBC_SHA]) } in
  let st,nsc,fch   = FlexClientHello.send(st,fch) in
  let log = log @| fch.payload in
...
cfuns := (162,tr162)::!cfuns;


let tr163 st chain salg skey = 
  let log = empty_bytes in
  let fch = {FlexConstants.nullFClientHello with ciphersuites = Some([TLS_RSA_WITH_AES_128_CBC_SHA]) } in
  let st,nsc,fch   = FlexClientHello.send(st,fch) in
  let log = log @| fch.payload in
...
cfuns := (163,tr163)::!cfuns;


let tr164 st chain salg skey = 
  let log = empty_bytes in
  let fch = {FlexConstants.nullFClientHello with ciphersuites = Some([TLS_RSA_WITH_AES_128_CBC_SHA]) } in
  let st,nsc,fch   = FlexClientHello.send(st,fch) in
  let log = log @| fch.payload in
...
cfuns := (164,tr164)::!cfuns;


let tr165 st chain salg skey = 
  let log = empty_bytes in
  let fch = {FlexConstants.nullFClientHello with ciphersuites = Some([TLS_RSA_WITH_AES_128_CBC_SHA]) } in
  let st,nsc,fch   = FlexClientHello.send(st,fch) in
  let log = log @| fch.payload in
